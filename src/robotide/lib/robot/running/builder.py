...
class TestSuiteBuilder(object):
    ...
    def _parse(self, path):
        try:
            ...
            raise DataError("Parsing '%s' failed: %s" % (path, str(err)))
    ...
    def _build_suite(self, data, parent_defaults=None):
        ...
        suite = TestSuite(name=data.name,
                          source=data.source,
                          doc=str(data.setting_table.doc),
    ...
    def _get_template(self, template):
        return str(template) if template.is_active() else None
...
class StepBuilder(object):
    ...
    def _create_templated(self, parent, data, template):
        ...
        template, args = self._format_template(str(template), args)
